...
// console.log('testttt', await authStore.adminAccess());
  ...
  if (to.meta.requiresAuth && !(await authStore.authenticated())) next({ name: 'signIn' })
  else if(to.meta.requiresAdmin  && await authStore.adminAccess() !== 'admin' ) next({ name: 'signIn' })
