                    ...
                    <table class='min-w-full leading-normal'>
                        ...
                        <tbody>
                            <tr  v-for="user in allUsers" :key="user?._id">
                                <td class='w-5/12  px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                                    <p class='text-gray-900 whitespace-no-wrap'>
                                        {{ user?.email }}
                                    </p>
                                </td>

                                <td class='w-4/12 px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                                    <p class='text-gray-900 whitespace-no-wrap'>
                                        {{ user?.role ? user.role : 'User' }}
                                    </p>
                                </td>
                                <td class=' 3/12  py-5 border-b border-gray-200 bg-white text-sm'>
                                    <span v-if="user?.role && user.role === 'requested'"
                                        ...
                                        <span class='relative' @click="handleRequest(user)">
                                            <Spinner_2 class="inset-0 " :text="''" v-if="loading"></Spinner_2>
